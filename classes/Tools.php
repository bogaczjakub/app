        ...
        if (!empty($links)) {
            ...
            if (is_array($links)) {
                foreach ($links as $link) {
                    $link_src = $type_select . $theme . DS . $link_type . DS . $link;
                    if (file_exists($link_src)) {
                        array_push($head_links[$link_type], self::returnAbsolutePath($link_src));
                    }
                }

            } elseif (is_string($links)) {
                $link_src = $type_select . $theme . DS . $link_type . DS . $links;
                if (file_exists($link_src)) {
                    array_push($head_links[$link_type], self::returnAbsolutePath($link_src));
                }
            }
